...
char* indexes(const char* path)
{
    ...
    char* newPath = malloc(sizeof(char) * (lenOfPath+1+10));
    char* newPathHTML = malloc(sizeof(char) * (lenOfPath+1+10));
    strcat(newPath, path);
    strcat(newPathHTML, path);
    char* indexPHP = strcat(newPath, "index.php");
    
    if(!access(indexPHP, R_OK)) {
        
        return indexPHP;
    }
    char* indexHTML = strcat(newPathHTML, "index.html");
    if(!access(indexHTML, R_OK)) {
        return indexHTML;
    }
    else {
        return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    *content = 0;
    BYTE* loaded = malloc(sizeof(BYTE)*1000);
    int i = 0;
    for(int c = fgetc(file); c != EOF; c = fgetc(file)) {
        if(i % 1000 == 0) {
            loaded = realloc(loaded, sizeof(BYTE)*(1000+i));
        }
        loaded[i] = c;
        i++;
    }
...
{
    // TODO
    for(int i=8; i<LimitRequestLine; i++) {
        abs_path[i] = '\0';
    }
    ...
    if (line[currentPlaceInLine+2] != 'H') {
        error(405);
        return false;
    }
